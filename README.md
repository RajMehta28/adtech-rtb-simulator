...
Full write-up of the guardrail extension — what it found, how it was built, and an honest bug caught before shipping — is in GUARDRAIL_EXTENSION.md.
...
docs/index.html is a single self-contained, no-build-step HTML page — zero external dependencies, every chart hand-rolled in inline SVG — built directly from this project's own output data. Every chart and KPI reflects the actual simulation run, not placeholder numbers, and every bar/point/segment is hoverable and click-to-drill-down into the underlying table row.
...
Python · pandas · numpy · scikit-learn · SQLite · matplotlib · vanilla JS/SVG (no charting library)
